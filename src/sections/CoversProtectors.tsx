...
export default function CoversProtectors() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response_data = await fetchProductsByCategory("covers_protectors");
  ...
  return (
    <section id="CoversProtectors" className="min-h-dvh px-4 py-8">
      ...
      <div className="covers_protectors flex flex-col items-center sm:flex-row sm:items-start flex-wrap  gap-2 pt-4">
        {items.length > 0
      ...
      {items.length > 0 ? (
        <div className="flex items-center justify-center my-8">
          <a
            href="/shop?category=covers_protectors"
